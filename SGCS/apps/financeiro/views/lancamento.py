...
from SGCS.apps.financeiro.forms import ContaPagarForm, ContaReceberForm, SaidaForm, EntradaForm
from SGCS.apps.financeiro.models import Lancamento, Saida, Entrada, MovimentoCaixa
from SGCS.apps.vendas.models import PedidoVenda
from SGCS.apps.compras.models import PedidoCompra
from SGCS.apps.estoque.models import SaidaEstoque, ItensMovimento, ProdutoEstocado
...
class MovimentoCaixaMixin(object):
    ...
    def adicionar_valor_movimento_caixa(self, lancamento, movimento, valor):
        ...
        elif isinstance(lancamento, PedidoVenda):
            movimento.saidas = movimento.saidas + valor
            movimento.saldo_final = movimento.saldo_final - valor
            movimento.save()
            for m in MovimentoCaixa.objects.filter(data_movimento__gt=movimento.data_movimento):
...
class AdicionarLancamentoBaseView(CustomCreateView, MovimentoCaixaMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if self.object.creditado:
                pass
            else:
                if isinstance(self.object, Entrada):
                    ...
                    paciente.save()
                    self.object.creditado = True
                else:
                    paciente = Paciente.objects.get(pk=self.object.fornecedor_id)
                    paciente.limite_de_credito = paciente.limite_de_credito - self.object.valor_total
                    paciente.save()
                    self.object.creditado = True
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if self.object.creditado:
                ...
                if isinstance(self.object, Saida):
                    paciente = Paciente.objects.get(pk=self.object.fornecedor_id)
                    paciente.limite_de_credito = paciente.limite_de_credito - self.object.valor_total
                    paciente.save()
                    self.object.creditado = True
...
class ContaReceberListView(LancamentoListBaseView):
    ...
    def get_queryset(self):
        return Paciente.objects.filter(limite_de_credito__lt=0)#super(ContaReceberListView, self).get_queryset(object=Entrada, status=['1', '2'])
...
class ContaReceberAtrasadasListView(LancamentoListBaseView):
    ...
    def get_queryset(self):
        return Paciente.objects.filter(limite_de_credito__lt=0)#Entrada.objects.filter(data_vencimento__lt=datetime.now().date(), status__in=['1', '2'])
...
class ContaReceberHojeListView(ContaReceberAtrasadasListView):
    ...
    def get_queryset(self):
        return Paciente.objects.filter(limite_de_credito__lt=0) #Entrada.objects.filter(data_vencimento=datetime.now().date(), status__in=['1', '2'])
...
class FaturarPedidoVendaView(CustomView, MovimentoCaixaMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for pagamento in pagamentos:
            entrada = Saida() # Trocando entrada por saída em Pedido de venda Faturado / Clínica
            print(pedido.cliente)
            entrada.fornecedor = pedido.cliente
            entrada.fornecedor = pedido.cliente
            entrada.status = '1'
...
class FaturarPedidoCompraView(CustomView, MovimentoCaixaMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for pagamento in pagamentos:
            saida = Saida()
            print(pedido.cliente)
            saida.fornecedor = pedido.cliente
            saida.status = '1'
