...
from SGCS.apps.base.custom_views import CustomCreateView, CustomListView, CustomUpdateView

from SGCS.apps.cadastro.forms import PessoaJuridicaForm, PessoaFisicaForm, EnderecoFormSet, FazendaFormSet, TelefoneFormSet, EmailFormSet, \
    SiteFormSet, BancoFormSet, DocumentoFormSet, ResponsavelFormSet
from SGCS.apps.cadastro.models import PessoaFisica, PessoaJuridica, Endereco, Fazenda, Responsavel, Telefone, Email, Site, Banco, Documento, Transportadora
...
class AdicionarPessoaView(CustomCreateView):
    ...
    def get(self, request, form, *args, **kwargs):
        ...
        responsavel_form = ResponsavelFormSet(prefix='responsavel_form')
        #responsavel_form.can_delete = False
    ...
    def post(self, request, form, *args, **kwargs):
        ...
        responsavel_form=ResponsavelFormSet(request.POST, prefix='responsavel_form')
        ...
        if form.is_valid():
            ...
            if (all(formset.is_valid() for formset in formsets) and
                ...
                responsavel_form.instance = self.object
                resp = responsavel_form.save()
...
class EditarPessoaView(CustomUpdateView):
    ...
    def get(self, request, form, *args, **kwargs):
        ...
        responsavel_form = ResponsavelFormSet(
            instance=self.object, prefix='responsavel_form')
    ...
    def post(self, request, form, *args, **kwargs):
        ...
        responsavel_form = ResponsavelFormSet(
            request.POST, prefix='responsavel_form', instance=self.object)
        fazenda_form = FazendaFormSet(
        ...
        if form.is_valid():
            ...
            if (all(formset.is_valid() for formset in formsets) and
                ...
                if self.object.tipo_pessoa == 'PJ':
                    ...
                    paciente = PessoaFisica.objects.filter(
                        pessoa_id=self.object.pk)
                    paciente.desativar = True
                else:
